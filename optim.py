...
for i in N:
    for j in N:
        D[i,j] = matrix[N.index(i)][N.index(j)]


#ゴミ集積所置くか否か(0-1変数):C
c = []
for i in range(0,NN):
    c.append(pulp.LpVariable("c({:})".format(i), 0, 2, pulp.LpInteger))
...
# c = LpVariable('地点iに集積所を置くか置かないか', cat=LpBinary)
# x = LpVariable('地点1から地点jに行くか行かないか', cat=LpBinary)

##定式化ゾーン
#目的関数
#各家から集積所までの合計距離最小化
problem += sum(x[i,j]*D[i,j] for i in H for j in N)

#制約条件
#ゴミ集積所の数は10個まで
problem += sum(c[i] for i in N) <= 42

#各家は1つの(集積所にしか行かない)
for i in H:
    problem += sum(x[i,j] for j in N) == 1

#家以外のノードからはゴミは出ない
problem += sum(x[i,j] for i in HN for j in N) == 0
...
#各集積所につき10戸までゴミを集められる
for j in N:
    problem += sum(x[i,j] for i in H) <= 10 * c[j]
...
for i in N:
    print("{:} = {:},  ".format(c[i].name, c[i].value()), end="")
    print("")
